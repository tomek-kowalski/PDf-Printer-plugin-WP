...
foreach ($items as $key => $values) {
    ...
    if (count($gallery_image_urls) > 0) {
        foreach ($gallery_image_urls as $gallery_image_url) {
            if ($gallery_image_url) {
                ...
                $pdf->Image($gallery_image_url, 150, $pdf->GetY()+41,40, 55);
    ...
    if (!empty($pic_url)) {
        $pdf->Image($pic_url, 15, $pdf->GetY()-5, 45, 45);
    }

    $pdf->setTextColor(66, 40, 14);
    $pdf->SetFont('dejavusans','',8);
    $pdf->SetX(60);
    $pdf->MultiCell(85, 4,$desc,0,'L');
    $pdf->Ln(7);
    $pdf->setTextColor(66, 40, 14);
...
foreach ($category_thumbnails as $thumbnail_id) {
    ...
    $pdf->SetX($pdf->getX()+2);
    $pdf->Image($thumbnail_url, $pdf->GetX()+39,$pdf->GetY(), 7,7); 
    $pdf->SetX($pdf->GetX() + 8);
}
$pdf->ln(4);
    $firstElements[] = $values;
    } 
        ...
        if (count($gallery_image_urls) > 0) {
            foreach ($gallery_image_urls as $gallery_image_url) {
                if ($gallery_image_url) {
                    ...
                    $pdf->Image($gallery_image_url, 150, $pdf->GetY()+12,40, 55);
